...
function solution(need, plan) {
	let answer = 'YES'
	let queue = need.split('') //memo 필수과목을 queue 배열에 저장

	for (let v of plan) {
		//memo queue 배열에서 현재과목에 일치하는 값이 있는지 검사
		if (queue.includes(v)) {
			//memo 먼저 queue에 값을 빼주고 필수과목의 순서가 맞지 않는다면 리턴
			if (v !== queue.shift()) return 'NO'
		}
	}
	//memo queue 배열이 비어있지 않다면 필수과목을 모두 이수 못했다는 뜻
...
let a = 'CBA'
console.log(solution(a, 'CBDAGE')) //* YES
console.log(solution(a, 'CGEADB')) //* NO
